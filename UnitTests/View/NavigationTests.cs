...
namespace Terminal.Gui.ViewTests {
	public class NavigationTests {
		...
		public void FocusNearestView_Ensure_Focus_Ordered ()
		{
			...
			top.ProcessKeyPressed (new (ConsoleDriverKey.Tab));
			...
			top.ProcessKeyPressed (new (ConsoleDriverKey.Tab | ConsoleDriverKey.ShiftMask));
			Assert.Equal ("FrameSubview", top.MostFocused.Text);
			top.ProcessKeyPressed (new (ConsoleDriverKey.Tab | ConsoleDriverKey.ShiftMask));
			Assert.Equal ($"WindowSubview", top.MostFocused.Text);
			top.Dispose ();
		...
		public void Enabled_False_Sets_HasFocus_To_False ()
		{
			...
			view.ProcessKeyPressed (new (ConsoleDriverKey.Space));
		...
		public void Enabled_Sets_Also_Sets_Subviews ()
		{
			...
			Application.Iteration += (s, a) => {
				...
				win.ProcessKeyPressed (new (ConsoleDriverKey.Enter));
				Assert.True (wasClicked);
				...
				win.Enabled = false;
				button.ProcessKeyPressed (new (ConsoleDriverKey.Enter));
				Assert.False (wasClicked);
				button.MouseEvent (new MouseEvent () { Flags = MouseFlags.Button1Clicked });
				Assert.False (wasClicked);
		...
		public void CanFocus_Sets_To_False_On_Single_View_Focus_View_On_Another_Toplevel ()
		{
			...
			Assert.True (Application.Top.ProcessKeyPressed (new (ConsoleDriverKey.Tab)));
			Assert.True (view1.CanFocus);
		...
		public void CanFocus_Sets_To_False_With_Two_Views_Focus_Another_View_On_The_Same_Toplevel ()
		{
			...
			Assert.True (Application.Top.ProcessKeyPressed (new (ConsoleDriverKey.Tab | ConsoleDriverKey.CtrlMask)));
			Assert.True (Application.Top.ProcessKeyPressed (new (ConsoleDriverKey.Tab | ConsoleDriverKey.CtrlMask)));
			Assert.True (view1.CanFocus);
			Assert.False (view1.HasFocus); // Only one of the most focused toplevels view can have focus
			Assert.True (view2.CanFocus);
		...
		public void HotKey_Will_Invoke_KeyPressed_Only_For_The_MostFocused_With_Top_KeyPress_Event ()
		{
			...
			var sb = new StatusBar (new StatusItem [] {
				new StatusItem(ConsoleDriverKey.CtrlMask | ConsoleDriverKey.Q, "~^Q~ Quit", () => sbQuiting = true )
			});
...
// This test is now invalid because `win` is focused, so it will receive the keypress
		...
		public void FocusNext_Does_Not_Throws_If_A_View_Was_Removed_From_The_Collection ()
		{
			...
			Assert.True (top1.ProcessKeyPressed (new (ConsoleDriverKey.Tab | ConsoleDriverKey.CtrlMask)));
			Assert.True (top1.HasFocus);
			Assert.False (view1.HasFocus);
			Assert.True (view2.HasFocus);
			Assert.True (removed);
			...
			var exception = Record.Exception (() => top1.ProcessKeyPressed (new (ConsoleDriverKey.Tab | ConsoleDriverKey.CtrlMask)));
			Assert.Null (exception);
			Assert.True (removed);
			Assert.Null (view3);
		}
