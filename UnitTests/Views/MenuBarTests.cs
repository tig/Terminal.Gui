...
public class MenuBarTests {
	...
	public void MenuOpening_MenuOpened_MenuClosing_Events ()
	{
		...
		Assert.True (menu.ProcessKeyPressed (new KeyEventArgs (menu.Key)));
...
";
	...
	public void MenuOpened_On_Disabled_MenuItem ()
	{
		...
		Assert.True (mCurrent.ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.CursorDown)));
		Assert.True (menu.IsMenuOpen);
		Assert.Equal ("_File", miCurrent.Parent.Title);
		Assert.Equal ("_Save", miCurrent.Title);

		Assert.True (mCurrent.ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.CursorUp)));
		Assert.True (menu.IsMenuOpen);
		Assert.Equal ("_File", miCurrent.Parent.Title);
		Assert.Equal ("_New", miCurrent.Title);
	...
	public void KeyBindings_Navigation_Commands (string expectedBarTitle, string expectedItemTitle, string expectedAction, params ConsoleDriverKey [] keys)
	{
		...
		foreach (var key in keys) {
			Application.Top.ProcessKeyPressed (new KeyEventArgs (key, KeyBindingScope.HotKey));
	...
	public void KeyBindings_Shortcut_Commands (string expectedAction, params ConsoleDriverKey [] keys)
	{
		...
		foreach (var key in keys) {
			Assert.True (Application.Top.ProcessKeyPressed (new KeyEventArgs (key, KeyBindingScope.HotKey)));
			Application.MainLoop.RunIteration ();
		}
		Assert.Equal (expectedAction, miAction);
	}
	...
	public void ShortCut_Activates ()
	{
		...
		Application.OnKeyPressed (new KeyEventArgs (ConsoleDriverKey.S | ConsoleDriverKey.CtrlMask, KeyBindingScope.HotKey));
		Application.MainLoop.RunIteration ();
...
";
		...
		Assert.True (Application.Top.Subviews [1].ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.CursorDown)));
		Application.Top.Draw ();
		expected = @"
 Numbers                 
┌────────┐               
...
";
		...
		Assert.True (Application.Top.Subviews [2].ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.CursorLeft)));
		Application.Top.Draw ();
		expected = @"
 Numbers  
┌────────┐
...
";
		...
		Assert.True (Application.Top.Subviews [1].ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.Esc)));
		Application.Top.Draw ();
		expected = @"
 Numbers
";
...
";
		...
		Assert.True (Application.Top.Subviews [1].ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.CursorDown)));
		Assert.True (Application.Top.Subviews [1].ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.Enter)));
		Application.Top.Draw ();
		expected = @"
 Numbers       
┌─────────────┐
...
";
		...
		Assert.True (Application.Top.Subviews [2].ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.Enter)));
		Application.Top.Draw ();
		expected = @"
 Numbers  
┌────────┐
...
";
	...
	public void HotKey_MenuBar_OnKeyDown_OnKeyUp_ProcessKeyPressed ()
	{
		...
		Assert.False (Application.Top.ProcessKeyDown (new KeyEventArgs (ConsoleDriverKey.AltMask)));
		Assert.False (Application.Top.ProcessKeyDown (new KeyEventArgs (ConsoleDriverKey.AltMask)));
		Assert.True (Application.Top.ProcessKeyUp (new KeyEventArgs (ConsoleDriverKey.AltMask)));
		Assert.True (menu.IsMenuOpen);
		Application.Top.Draw ();
		string expected = @"
 File  Edit
";
		...
		Assert.True (Application.Top.ProcessKeyDown (new KeyEventArgs (ConsoleDriverKey.N)));
		Application.MainLoop.RunIteration ();
		Assert.False (newAction); // not yet, hot keys don't work if the item is not visible

		Assert.True (Application.Top.ProcessKeyDown (new KeyEventArgs (ConsoleDriverKey.F)));
		Application.MainLoop.RunIteration ();
		Assert.True (Application.Top.ProcessKeyDown (new KeyEventArgs (ConsoleDriverKey.N)));
		Application.MainLoop.RunIteration ();
...
";
		...
		Assert.True (Application.Top.ProcessKeyDown (new KeyEventArgs (ConsoleDriverKey.CursorRight)));
		Assert.True (Application.Top.ProcessKeyDown (new KeyEventArgs (ConsoleDriverKey.C)));
		Application.MainLoop.RunIteration ();
	...
	public void HotKey_MenuBar_ProcessKeyPressed_Menu_ProcessKey ()
	{
		...
		Assert.True (menu.ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.AltMask | ConsoleDriverKey.F)));
		Assert.True (menu.IsMenuOpen);
		Application.Top.Draw ();
		TestHelpers.AssertDriverContentsAre (expectedMenu.expectedSubMenuOpen (0), _output);

		Assert.True (Application.Top.Subviews [1].ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.N)));
		Application.MainLoop.RunIteration ();
		Assert.True (newAction);

		Assert.True (menu.ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.AltMask | ConsoleDriverKey.E)));
		Assert.True (menu.IsMenuOpen);
		Application.Top.Draw ();
		TestHelpers.AssertDriverContentsAre (expectedMenu.expectedSubMenuOpen (1), _output);

		Assert.True (Application.Top.Subviews [1].ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.C)));
		Application.MainLoop.RunIteration ();
		Assert.True (copyAction);
	}
	...
	public void MenuBar_Position_And_Size_With_HotKeys_Is_The_Same_As_Without_HotKeys ()
	{
		...
		// Open second
		Assert.True (Application.Top.Subviews [1].ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.CursorRight)));
		Assert.True (menu.IsMenuOpen);
		Application.Top.Draw ();
		TestHelpers.AssertDriverContentsAre (expectedMenu.expectedSubMenuOpen (1), _output);
	...
	public void Parent_MenuItem_Stay_Focused_If_Child_MenuItem_Is_Empty_By_Keyboard ()
	{
		...
		Assert.True (menu._openMenu.ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.CursorRight)));
		Assert.True (menu.IsMenuOpen);
		Assert.False (tf.HasFocus);
		Application.Top.Draw ();
		TestHelpers.AssertDriverContentsAre (expectedMenu.expectedSubMenuOpen (1), _output);
		...
		// Left - Edit
		Assert.True (menu._openMenu.ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.CursorLeft)));
		Assert.True (menu.IsMenuOpen);
		Assert.False (tf.HasFocus);
		Application.Top.Draw ();
		TestHelpers.AssertDriverContentsAre (expectedMenu.expectedSubMenuOpen (1), _output);
	...
	public void Key_Open_And_Close_The_MenuBar ()
	{
		...
		Assert.True (Application.Top.ProcessKeyPressed (new KeyEventArgs (menu.Key, KeyBindingScope.HotKey)));
		Assert.True (menu.IsMenuOpen);
		Assert.True (Application.Top.ProcessKeyPressed (new KeyEventArgs (menu.Key, KeyBindingScope.HotKey)));
		Assert.False (menu.IsMenuOpen);

		menu.Key = ConsoleDriverKey.F10 | ConsoleDriverKey.ShiftMask;
		Assert.False (Application.Top.ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.F9, KeyBindingScope.HotKey)));
		Assert.False (menu.IsMenuOpen);

		Assert.True (Application.Top.ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.F10 | ConsoleDriverKey.ShiftMask, KeyBindingScope.HotKey)));
		Assert.True (menu.IsMenuOpen);
		Assert.True (Application.Top.ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.F10 | ConsoleDriverKey.ShiftMask, KeyBindingScope.HotKey)));
		Assert.False (menu.IsMenuOpen);
	}
...
#if SUPPORT_ALT_TO_ACTIVATE_MENU
		Assert.True (Application.OnKeyUp (new KeyEventArgs (ConsoleDriverKey.AltMask))); // changed to true because Alt activates menu bar
#endif
		Assert.True (menu.ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.CursorRight)));
		Assert.True (menu.ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.CursorRight)));
	}
...
└──────────────────────────────────────┘", _output);
			...
			Assert.True (win.ProcessKeyPressed (new KeyEventArgs (menu.Key, KeyBindingScope.HotKey)));
			top.Draw ();
			TestHelpers.AssertDriverContentsWithFrameAre (@"
┌──────────────────────────────────────┐
│ File  Edit                           │
...
└──────────────────────────────────────┘", _output);
			...
			Assert.True (menu.ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.CursorRight, KeyBindingScope.HotKey)));
			Application.Refresh ();
			TestHelpers.AssertDriverContentsWithFrameAre (@"
┌──────────────────────────────────────┐
│ File  Edit                           │
...
└──────────────────────────────────────┘", _output);
			...
			Assert.True (menu._openMenu.ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.CursorRight, KeyBindingScope.HotKey)));
			top.Draw ();
			TestHelpers.AssertDriverContentsWithFrameAre (@"
┌──────────────────────────────────────┐
│ File  Edit                           │
...
└──────────────────────────────────────┘", _output);
			...
			Assert.True (top.ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.F9, KeyBindingScope.HotKey)));
			top.Draw ();
			TestHelpers.AssertDriverContentsWithFrameAre (@"
┌──────────────────────────────────────┐
│ File  Edit                           │
...
└──────────────────────────────────────┘", _output);
			...
			Assert.True (top.Subviews [0].ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.CursorRight, KeyBindingScope.HotKey)));
			Application.Refresh ();
			TestHelpers.AssertDriverContentsWithFrameAre (@"
┌──────────────────────────────────────┐
│ File  Edit                           │
...
└──────────────────────────────────────┘", _output);
			...
			Assert.True (((MenuBar)top.Subviews [0])._openMenu.ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.CursorRight, KeyBindingScope.HotKey)));
			top.Draw ();
			TestHelpers.AssertDriverContentsWithFrameAre (@"
┌──────────────────────────────────────┐
│ File  Edit                           │
...
└──────────────────────────────────────┘", _output);
	...
	public void AllowNullChecked_Get_Set ()
	{
		...
		Assert.True (menu.ProcessKeyPressed (new KeyEventArgs (menu.Key)));
		Assert.True (menu._openMenu.ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.Enter)));
		Application.MainLoop.RunIteration ();
...
";
	...
	public void MenuBarItem_Children_Null_Does_Not_Throw ()
	{
		...
		var exception = Record.Exception (() => menu.ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.Space)));
	...
	public void CanExecute_HotKey ()
	{
		...
		Assert.True (top.ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.F | ConsoleDriverKey.AltMask, KeyBindingScope.HotKey)));
		Assert.True (top.ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.N | ConsoleDriverKey.AltMask, KeyBindingScope.HotKey)));
		Application.MainLoop.RunIteration ();
		Assert.NotNull (win);
	...
	public void Separators_Does_Not_Throws_Pressing_Menu_Shortcut ()
	{
		...
		var exception = Record.Exception (() => Assert.True (menu.ProcessKeyPressed (new KeyEventArgs (ConsoleDriverKey.AltMask | ConsoleDriverKey.Q))));
		Assert.Null (exception);
	}
}