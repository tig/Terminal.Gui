...
namespace Terminal.Gui.ViewsTests {
	public class DateFieldTests {
		...
		public void KeyBindings_Command ()
		{
			...
			df.ReadOnly = true;
			Assert.True (df.ProcessKeyPressed (new (ConsoleDriverKey.DeleteChar)));
			Assert.Equal (" 12/12/1971", df.Text);
			df.ReadOnly = false;
			Assert.True (df.ProcessKeyPressed (new (ConsoleDriverKey.D | ConsoleDriverKey.CtrlMask)));
			Assert.Equal (" 02/12/1971", df.Text);
			df.CursorPosition = 4;
			df.ReadOnly = true;
			Assert.True (df.ProcessKeyPressed (new (ConsoleDriverKey.Delete)));
			Assert.Equal (" 02/12/1971", df.Text);
			df.ReadOnly = false;
			Assert.True (df.ProcessKeyPressed (new (ConsoleDriverKey.Backspace)));
			Assert.Equal (" 02/02/1971", df.Text);
			Assert.True (df.ProcessKeyPressed (new (ConsoleDriverKey.Home)));
			Assert.Equal (1, df.CursorPosition);
			Assert.True (df.ProcessKeyPressed (new (ConsoleDriverKey.End)));
			Assert.Equal (10, df.CursorPosition);
			Assert.True (df.ProcessKeyPressed (new (ConsoleDriverKey.A | ConsoleDriverKey.CtrlMask)));
			Assert.Equal (1, df.CursorPosition);
			Assert.True (df.ProcessKeyPressed (new (ConsoleDriverKey.E | ConsoleDriverKey.CtrlMask)));
			Assert.Equal (10, df.CursorPosition);
			Assert.True (df.ProcessKeyPressed (new (ConsoleDriverKey.CursorLeft)));
			Assert.Equal (9, df.CursorPosition);
			Assert.True (df.ProcessKeyPressed (new (ConsoleDriverKey.CursorRight)));
			Assert.Equal (10, df.CursorPosition);
			// Non-numerics are ignored
			Assert.True (df.ProcessKeyPressed (new (ConsoleDriverKey.A)));
			df.ReadOnly = true;
			df.CursorPosition = 1;
			Assert.True (df.ProcessKeyPressed (new (ConsoleDriverKey.D1)));
			Assert.Equal (" 02/02/1971", df.Text);
			df.ReadOnly = false;
			Assert.True (df.ProcessKeyPressed (new (ConsoleDriverKey.D1)));
