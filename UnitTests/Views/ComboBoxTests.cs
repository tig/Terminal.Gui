...
namespace Terminal.Gui.ViewsTests {
	public class ComboBoxTests {
		...
		public void EnsureKeyEventsDoNotCauseExceptions ()
		{
			...
			foreach (var key in (ConsoleDriverKey[])Enum.GetValues (typeof (ConsoleDriverKey))) {
				Assert.Null (Record.Exception (() => comboBox.ProcessKeyPressed (new (key))));
			}
		}
		...
		public void KeyBindings_Command ()
		{
			...
			Assert.True (cb.ProcessKeyPressed (new (ConsoleDriverKey.Enter)));
			Assert.False (opened);
			cb.Text = "Tw";
			Assert.True (cb.ProcessKeyPressed (new (ConsoleDriverKey.Enter)));
			Assert.True (opened);
			...
			cb.SetSource (null);
			Assert.False (cb.ProcessKeyPressed (new (ConsoleDriverKey.Enter)));
			Assert.True (cb.ProcessKeyPressed (new (ConsoleDriverKey.F4))); // with no source also expand empty
			Assert.True (cb.IsShow);
			Assert.Equal (-1, cb.SelectedItem);
			cb.SetSource (source);
			cb.Text = "";
			Assert.True (cb.ProcessKeyPressed (new (ConsoleDriverKey.F4))); // collapse
			Assert.False (cb.IsShow);
			Assert.True (cb.ProcessKeyPressed (new (ConsoleDriverKey.F4))); // expand
			Assert.True (cb.IsShow);
			cb.Collapse ();
			Assert.False (cb.IsShow);
			Assert.True (cb.HasFocus);
			Assert.True (cb.ProcessKeyPressed (new (ConsoleDriverKey.CursorDown))); // losing focus
			Assert.False (cb.IsShow);
			...
			Assert.True (cb.ProcessKeyPressed (new (ConsoleDriverKey.CursorDown)));
			Assert.True (cb.IsShow);
			Assert.Equal (1, cb.SelectedItem);
			Assert.Equal ("Two", cb.Text);
			Assert.True (cb.ProcessKeyPressed (new (ConsoleDriverKey.CursorDown)));
			...
			Assert.Equal (2, cb.SelectedItem);
			Assert.Equal ("Three", cb.Text);
			Assert.True (cb.ProcessKeyPressed (new (ConsoleDriverKey.CursorUp)));
			Assert.True (cb.IsShow);
			Assert.Equal (1, cb.SelectedItem);
			Assert.Equal ("Two", cb.Text);
			Assert.True (cb.ProcessKeyPressed (new (ConsoleDriverKey.CursorUp)));
...
", output);
			...
			Assert.True (cb.ProcessKeyPressed (new (ConsoleDriverKey.PageDown)));
			Assert.True (cb.IsShow);
			Assert.Equal (1, cb.SelectedItem);
			Assert.Equal ("Two", cb.Text);
			Application.Begin (Application.Top);
...
Three     
", output);
			Assert.True (cb.ProcessKeyPressed (new (ConsoleDriverKey.PageUp)));
			Assert.True (cb.IsShow);
			Assert.Equal (1, cb.SelectedItem);
			Assert.Equal ("Two", cb.Text);
			Assert.True (cb.ProcessKeyPressed (new (ConsoleDriverKey.PageUp)));
			...
			Assert.Equal (0, cb.SelectedItem);
			Assert.Equal ("One", cb.Text);
			Assert.True (cb.ProcessKeyPressed (new (ConsoleDriverKey.F4)));
			Assert.False (cb.IsShow);
			Assert.Equal (0, cb.SelectedItem);
			Assert.Equal ("One", cb.Text);
			Assert.True (cb.ProcessKeyPressed (new (ConsoleDriverKey.End)));
			Assert.False (cb.IsShow);
			Assert.Equal (0, cb.SelectedItem);
			Assert.Equal ("One", cb.Text);
			Assert.True (cb.ProcessKeyPressed (new (ConsoleDriverKey.Home)));
			Assert.False (cb.IsShow);
			Assert.Equal (0, cb.SelectedItem);
			Assert.Equal ("One", cb.Text);
			Assert.True (cb.ProcessKeyPressed (new (ConsoleDriverKey.F4)));
			...
			Assert.Equal (0, cb.SelectedItem);
			Assert.Equal ("One", cb.Text);
			Assert.True (cb.ProcessKeyPressed (new (ConsoleDriverKey.Esc)));
			Assert.False (cb.IsShow);
			Assert.Equal (0, cb.SelectedItem);
			...
			Assert.Equal (0, cb.SelectedItem);
			Assert.Equal ("One", cb.Text);
			Assert.True (cb.ProcessKeyPressed (new (ConsoleDriverKey.U | ConsoleDriverKey.CtrlMask)));
			Assert.True (cb.HasFocus);
			Assert.True (cb.IsShow);
			Assert.Equal (0, cb.SelectedItem);
			Assert.Equal ("", cb.Text);
		...
		public void HideDropdownListOnClick_True_OpenSelectedItem_With_Mouse_And_Key_And_Mouse ()
		{
			...
			Assert.True (cb.Subviews [1].ProcessKeyPressed (new (ConsoleDriverKey.CursorDown)));
			Assert.True (cb.MouseEvent (new MouseEvent {
				X = cb.Bounds.Right - 1,
				Y = 0,
				Flags = MouseFlags.Button1Pressed
			...
			Assert.True (cb.Subviews [1].ProcessKeyPressed (new (ConsoleDriverKey.CursorUp)));
			Assert.True (cb.MouseEvent (new MouseEvent {
				X = cb.Bounds.Right - 1,
				Y = 0,
				Flags = MouseFlags.Button1Pressed
		...
		public void HideDropdownListOnClick_True_OpenSelectedItem_With_Mouse_And_Key_CursorDown_And_Esc ()
		{
			...
			Assert.True (cb.Subviews [1].ProcessKeyPressed (new (ConsoleDriverKey.Enter)));
