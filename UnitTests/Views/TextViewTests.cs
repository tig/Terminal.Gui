...
public class TextViewTests {
	...
	public void WordForward_With_No_Selection ()
	{
		...
		while (_textView.CursorPosition.X < _textView.Text.Length) {
			_textView.ProcessKeyPressed (new (ConsoleDriverKey.CursorRight | ConsoleDriverKey.CtrlMask));
			switch (iteration) {
			case 0:
	...
	public void WordBackward_With_No_Selection ()
	{
		...
		while (_textView.CursorPosition.X > 0) {
			_textView.ProcessKeyPressed (new (ConsoleDriverKey.CursorLeft | ConsoleDriverKey.CtrlMask));
			switch (iteration) {
			case 0:
	...
	public void Selected_Text_Shows ()
	{
		...
		_textView.ProcessKeyPressed (new (ConsoleDriverKey.CursorRight | ConsoleDriverKey.CtrlMask | ConsoleDriverKey.ShiftMask));
	...
	public void WordBackward_With_Selection ()
	{
		...
		while (_textView.CursorPosition.X > 0) {
			_textView.ProcessKeyPressed (new (ConsoleDriverKey.CursorLeft | ConsoleDriverKey.CtrlMask | ConsoleDriverKey.ShiftMask));
			switch (iteration) {
			case 0:
				Assert.Equal (31, _textView.CursorPosition.X);
				Assert.Equal (0, _textView.CursorPosition.Y);
	...
	public void Kill_To_End_Delete_Forwards_Copy_To_The_Clipboard_And_Paste ()
	{
		...
		while (!iterationsFinished) {
			switch (iteration) {
			case 0:
				_textView.ProcessKeyPressed (new (ConsoleDriverKey.K | ConsoleDriverKey.CtrlMask));
				Assert.Equal (0, _textView.CursorPosition.X);
				Assert.Equal (0, _textView.CursorPosition.Y);
				...
				break;
			case 1:
				_textView.ProcessKeyPressed (new (ConsoleDriverKey.DeleteChar | ConsoleDriverKey.CtrlMask | ConsoleDriverKey.ShiftMask));
				Assert.Equal (0, _textView.CursorPosition.X);
				Assert.Equal (0, _textView.CursorPosition.Y);
			...
			case 2:
				...
				// Paste
				_textView.ProcessKeyPressed (new (ConsoleDriverKey.Y | ConsoleDriverKey.CtrlMask));
	...
	public void Kill_To_Start_Delete_Backwards_Copy_To_The_Clipboard_And_Paste ()
	{
		...
		while (!iterationsFinished) {
			switch (iteration) {
			case 0:
				_textView.ProcessKeyPressed (new (ConsoleDriverKey.K | ConsoleDriverKey.AltMask));
				Assert.Equal (0, _textView.CursorPosition.X);
				Assert.Equal (1, _textView.CursorPosition.Y);
				...
				break;
			case 1:
				_textView.ProcessKeyPressed (new (ConsoleDriverKey.Backspace | ConsoleDriverKey.CtrlMask | ConsoleDriverKey.ShiftMask));
				Assert.Equal (23, _textView.CursorPosition.X);
				Assert.Equal (0, _textView.CursorPosition.Y);
	...
	public void Kill_Delete_WordForward ()
	{
		...
		while (!iterationsFinished) {
			_textView.ProcessKeyPressed (new (ConsoleDriverKey.DeleteChar | ConsoleDriverKey.CtrlMask));
			switch (iteration) {
			case 0:
				Assert.Equal (0, _textView.CursorPosition.X);
				Assert.Equal (0, _textView.CursorPosition.Y);
	...
	public void Kill_Delete_WordBackward ()
	{
		...
		while (!iterationsFinished) {
			_textView.ProcessKeyPressed (new (ConsoleDriverKey.Backspace | ConsoleDriverKey.CtrlMask));
			switch (iteration) {
			case 0:
				Assert.Equal (22, _textView.CursorPosition.X);
				Assert.Equal (0, _textView.CursorPosition.Y);
	...
	public void Copy_Or_Cut_Null_If_No_Selection ()
	{
		_textView.SelectionStartColumn = 0;
		_textView.SelectionStartRow = 0;
		_textView.ProcessKeyPressed (new (ConsoleDriverKey.C | ConsoleDriverKey.CtrlMask)); // Copy
		Assert.Equal ("", _textView.SelectedText);
		_textView.ProcessKeyPressed (new (ConsoleDriverKey.W | ConsoleDriverKey.CtrlMask)); // Cut
		Assert.Equal ("", _textView.SelectedText);
	}
	...
	public void Copy_Or_Cut_And_Paste_With_Selection ()
	{
		...
		_textView.ProcessKeyPressed (new (ConsoleDriverKey.Y | ConsoleDriverKey.CtrlMask)); // Paste
		Assert.Equal ("TAB to jump between text fields.", _textView.Text);
		_textView.SelectionStartColumn = 20;
		_textView.SelectionStartRow = 0;
		_textView.ProcessKeyPressed (new (ConsoleDriverKey.W | ConsoleDriverKey.CtrlMask)); // Cut
	...
	public void Cut_Not_Allowed_If_ReadOnly_Is_True ()
	{
		...
		_textView.ProcessKeyPressed (new (ConsoleDriverKey.C | ConsoleDriverKey.CtrlMask)); // Copy
		Assert.Equal ("text", _textView.SelectedText);
		_textView.ProcessKeyPressed (new (ConsoleDriverKey.W | ConsoleDriverKey.CtrlMask)); // Selecting is set to false after Cut.
		Assert.Equal ("", _textView.SelectedText);
	...
	public void TextChanged_Event_NoFires_OnTyping ()
	{
		...
		_textView.ProcessKeyPressed (new (ConsoleDriverKey.Y | ConsoleDriverKey.ShiftMask));
		Assert.Equal (1, eventcount);
	...
	public void Used_Is_True_By_Default ()
	{
		_textView.CursorPosition = new Point (10, 0);
		Assert.Equal ("TAB to jump between text fields.", _textView.Text);
		_textView.ProcessKeyPressed (new ((ConsoleDriverKey)0x75)); // u
		Assert.Equal ("TAB to jumup between text fields.", _textView.Text);
		_textView.ProcessKeyPressed (new ((ConsoleDriverKey)0x73)); // s
		Assert.Equal ("TAB to jumusp between text fields.", _textView.Text);
		_textView.ProcessKeyPressed (new ((ConsoleDriverKey)0x65)); // e
		Assert.Equal ("TAB to jumusep between text fields.", _textView.Text);
		_textView.ProcessKeyPressed (new ((ConsoleDriverKey)0x64)); // d
	...
	public void TabWidth_Setting_To_Zero_Keeps_AllowsTab ()
	{
		...
		Assert.True (_textView.AllowsReturn);
		Assert.True (_textView.Multiline);
		_textView.ProcessKeyPressed (new (ConsoleDriverKey.Tab));
		Assert.Equal ("\tTAB to jump between text fields.", _textView.Text);
		_textView.ProcessKeyPressed (new (ConsoleDriverKey.Tab | ConsoleDriverKey.ShiftMask));
		Assert.Equal ("TAB to jump between text fields.", _textView.Text);
	}
	...
	public void AllowsReturn_Setting_To_True_Changes_Multiline_To_True_If_It_Is_False ()
	{
		...
		Assert.Equal (4, _textView.TabWidth);
		Assert.True (_textView.AllowsTab);
		_textView.ProcessKeyPressed (new (ConsoleDriverKey.Enter));
		Assert.Equal (Environment.NewLine +
	...
	public void Tab_Test_Follow_By_CursorLeft_And_Then_Follow_By_CursorRight ()
	{
		...
		Application.Iteration += (s, a) => {
			...
			while (col > 0) {
				col--;
				_textView.ProcessKeyPressed (new (ConsoleDriverKey.CursorLeft));
				Assert.Equal (new Point (col, 0), _textView.CursorPosition);
				leftCol = GetLeftCol (leftCol);
				Assert.Equal (leftCol, _textView.LeftColumn);
			}
			while (col < 100) {
				col++;
				_textView.ProcessKeyPressed (new (ConsoleDriverKey.CursorRight));
				Assert.Equal (new Point (col, 0), _textView.CursorPosition);
				leftCol = GetLeftCol (leftCol);
				Assert.Equal (leftCol, _textView.LeftColumn);
			}
	...
	public void Tab_Test_Follow_By_Home_And_Then_Follow_By_End_And_Then_Follow_By_BackTab_With_Text ()
	{
		...
		Application.Iteration += (s, a) => {
			...
			_textView.ProcessKeyPressed (new (ConsoleDriverKey.Home));
			col = 0;
			Assert.Equal (new Point (col, 0), _textView.CursorPosition);
			leftCol = 0;
			Assert.Equal (leftCol, _textView.LeftColumn);

			_textView.ProcessKeyPressed (new (ConsoleDriverKey.End));
...
", _output);
		...
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.Backspace)));
		Application.Refresh ();
		Assert.Equal (0, tv.LeftColumn);
		TestHelpers.AssertDriverContentsAre (@"
aaa
...
", _output);
	...
	public void BottomOffset_Sets_To_Zero_Adjust_TopRow ()
	{
		...
		tv.ProcessKeyPressed (new (ConsoleDriverKey.CtrlMask | ConsoleDriverKey.End));
	...
	public void RightOffset_Sets_To_Zero_Adjust_leftColumn ()
	{
		...
		tv.ProcessKeyPressed (new (ConsoleDriverKey.End));
	...
	[InlineData (ConsoleDriverKey.DeleteChar)]
	public void WordWrap_Draw_Typed_Keys_After_Text_Is_Deleted (ConsoleDriverKey del)
	{
		Application.Top.Add (_textView);
...
Line 2.", _output);
		...
		Assert.True (_textView.ProcessKeyPressed (new (ConsoleDriverKey.End | ConsoleDriverKey.ShiftMask)));
		Assert.Equal ("Line 1.", _textView.SelectedText);

		Assert.True (_textView.ProcessKeyPressed (new (del)));
		Application.Refresh ();
		TestHelpers.AssertDriverContentsWithFrameAre ("Line 2.", _output);

		Assert.True (_textView.ProcessKeyPressed (new (ConsoleDriverKey.H | ConsoleDriverKey.ShiftMask)));
...
Line 2.", _output);
	...
	public void LeftColumn_Add_One_If_Text_Length_Is_Equal_To_Width ()
	{
		...
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.CursorRight)));
	...
	public void KeyBindings_Command ()
	{
		...
		tv.CanFocus = false;
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.CursorLeft)));
		tv.CanFocus = true;
		Assert.False (tv.ProcessKeyPressed (new (ConsoleDriverKey.CursorLeft)));
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.CursorRight)));
		Assert.Equal (new Point (1, 0), tv.CursorPosition);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.End | ConsoleDriverKey.CtrlMask)));
		...
		Assert.False (tv.ProcessKeyPressed (new (ConsoleDriverKey.CursorRight)));
		Assert.NotNull (tv.Autocomplete);
		Assert.Empty (g.AllSuggestions);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.F | ConsoleDriverKey.ShiftMask)));
		tv.Draw ();
		...
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.Z | ConsoleDriverKey.CtrlMask)));
		tv.Draw ();
		Assert.Equal ($"This is the first line.{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.", tv.Text);
		Assert.Equal (new Point (23, 2), tv.CursorPosition);
		Assert.Empty (tv.Autocomplete.Suggestions);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.R | ConsoleDriverKey.CtrlMask)));
		tv.Draw ();
		Assert.Equal ($"This is the first line.{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.F", tv.Text);
		Assert.Equal (new Point (24, 2), tv.CursorPosition);
		Assert.Empty (tv.Autocomplete.Suggestions);
		...
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.Enter)));
		...
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.PageDown | ConsoleDriverKey.ShiftMask)));
		Assert.Equal (28, tv.GetCurrentLine ().Count);
		Assert.Equal (new Point (23, 2), tv.CursorPosition); // gets the previous length
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.Home | ConsoleDriverKey.CtrlMask)));
		Assert.Equal (Point.Empty, tv.CursorPosition);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.N | ConsoleDriverKey.CtrlMask)));
		Assert.Equal (new Point (0, 1), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.P | ConsoleDriverKey.CtrlMask)));
		Assert.Equal (new Point (0, 0), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.CursorDown)));
		Assert.Equal (new Point (0, 1), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.CursorUp)));
		Assert.Equal (new Point (0, 0), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.CursorDown | ConsoleDriverKey.ShiftMask)));
		Assert.Equal (new Point (0, 1), tv.CursorPosition);
		...
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.CursorUp | ConsoleDriverKey.ShiftMask)));
		Assert.Equal (new Point (0, 0), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.F | ConsoleDriverKey.CtrlMask)));
		Assert.Equal (new Point (1, 0), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.B | ConsoleDriverKey.CtrlMask)));
		Assert.Equal (new Point (0, 0), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.CursorRight)));
		...
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.CursorRight | ConsoleDriverKey.ShiftMask)));
		Assert.Equal (new Point (1, 0), tv.CursorPosition);
		Assert.Equal (1, tv.SelectedLength);
		Assert.Equal ("T", tv.SelectedText);
		Assert.True (tv.Selecting);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.CursorLeft | ConsoleDriverKey.ShiftMask)));
		Assert.Equal (new Point (0, 0), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		Assert.True (tv.Selecting);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.DeleteChar)));
		Assert.Equal ($"This is the first line.{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
		Assert.Equal (new Point (0, 0), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		...
		Assert.Equal ("", tv.SelectedText);
		Assert.False (tv.Selecting);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.D | ConsoleDriverKey.CtrlMask)));
		...
		Assert.Equal (new Point (0, 0), tv.CursorPosition);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.End)));
		Assert.Equal ($"is is the first line.{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
		Assert.Equal (new Point (21, 0), tv.CursorPosition);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.Delete)));
		Assert.Equal ($"is is the first line{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
		...
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.Home)));
		Assert.Equal (new Point (0, 0), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		Assert.False (tv.Selecting);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.End | ConsoleDriverKey.ShiftMask)));
		Assert.Equal (new Point (19, 0), tv.CursorPosition);
		Assert.Equal (19, tv.SelectedLength);
		Assert.Equal ("is is the first lin", tv.SelectedText);
		Assert.True (tv.Selecting);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.Home | ConsoleDriverKey.ShiftMask)));
		Assert.Equal (new Point (0, 0), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		Assert.True (tv.Selecting);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.E | ConsoleDriverKey.CtrlMask)));
		Assert.Equal (new Point (19, 0), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		Assert.False (tv.Selecting);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.A | ConsoleDriverKey.CtrlMask)));
		Assert.Equal (new Point (0, 0), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		Assert.False (tv.Selecting);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.K | ConsoleDriverKey.CtrlMask)));
		...
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.Y | ConsoleDriverKey.CtrlMask)));
		Assert.Equal ($"is is the first lin{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
		Assert.Equal (new Point (19, 0), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		...
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.DeleteChar | ConsoleDriverKey.CtrlMask | ConsoleDriverKey.ShiftMask)));
		Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
		Assert.Equal (new Point (0, 0), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		...
		Assert.False (tv.Selecting);
		Assert.Equal ("is is the first lin", Clipboard.Contents);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.K | ConsoleDriverKey.AltMask)));
		Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
		Assert.Equal (new Point (0, 0), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		...
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.Space | ConsoleDriverKey.CtrlMask)));
		Assert.Equal ($"is is the first lin{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
		Assert.Equal (new Point (19, 0), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		...
		Assert.Equal (0, tv.SelectionStartRow);
		tv.SelectionStartColumn = 0;
		Assert.True (tv.ProcessKeyPressed (new (((int)'C' + ConsoleDriverKey.AltMask))));
		Assert.Equal ($"is is the first lin{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
		Assert.Equal (new Point (19, 0), tv.CursorPosition);
		Assert.Equal (19, tv.SelectedLength);
		Assert.Equal ("is is the first lin", tv.SelectedText);
		...
		Assert.Equal (0, tv.SelectionStartColumn);
		Assert.Equal (0, tv.SelectionStartRow);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.C | ConsoleDriverKey.CtrlMask)));
		Assert.Equal ($"is is the first lin{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
		Assert.Equal (new Point (19, 0), tv.CursorPosition);
		Assert.Equal (19, tv.SelectedLength);
		Assert.Equal ("is is the first lin", tv.SelectedText);
		...
		Assert.Equal (0, tv.SelectionStartColumn);
		Assert.Equal (0, tv.SelectionStartRow);
		Assert.True (tv.ProcessKeyPressed (new (((int)'W' + ConsoleDriverKey.AltMask))));
		Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
		Assert.Equal (new Point (0, 0), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		...
		Assert.Equal (0, tv.SelectionStartRow);
		Assert.Equal ("is is the first lin", Clipboard.Contents);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.W | ConsoleDriverKey.CtrlMask)));
		Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
		Assert.Equal (new Point (0, 0), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		...
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.X | ConsoleDriverKey.CtrlMask)));
		Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
		Assert.Equal (new Point (0, 0), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		...
		Assert.Equal ("", tv.SelectedText);
		Assert.False (tv.Selecting);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.CtrlMask | ConsoleDriverKey.CursorLeft)));
		Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
		Assert.Equal (new Point (18, 2), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		Assert.False (tv.Selecting);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.CtrlMask | ConsoleDriverKey.CursorLeft | ConsoleDriverKey.ShiftMask)));
		Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
		Assert.Equal (new Point (12, 2), tv.CursorPosition);
		Assert.Equal (6, tv.SelectedLength);
		Assert.Equal ("third ", tv.SelectedText);
		Assert.True (tv.Selecting);
		Assert.True (tv.ProcessKeyPressed (new ((ConsoleDriverKey)((int)'B' + ConsoleDriverKey.AltMask))));
		Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
		Assert.Equal (new Point (8, 2), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		Assert.False (tv.Selecting);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.CtrlMask | ConsoleDriverKey.CursorRight)));
		Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
		Assert.Equal (new Point (12, 2), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		Assert.False (tv.Selecting);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.CtrlMask | ConsoleDriverKey.CursorRight | ConsoleDriverKey.ShiftMask)));
		Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
		Assert.Equal (new Point (18, 2), tv.CursorPosition);
		Assert.Equal (6, tv.SelectedLength);
		Assert.Equal ("third ", tv.SelectedText);
		Assert.True (tv.Selecting);
		Assert.True (tv.ProcessKeyPressed (new ((ConsoleDriverKey)((int)'F' + ConsoleDriverKey.AltMask))));
		Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
		...
		Assert.Equal ("", tv.SelectedText);
		Assert.False (tv.Selecting);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.DeleteChar | ConsoleDriverKey.CtrlMask)));
		Assert.Equal ($"This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
		Assert.Equal (new Point (0, 0), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		Assert.False (tv.Selecting); Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.End | ConsoleDriverKey.CtrlMask)));
		Assert.Equal ($"This is the second line.{Environment.NewLine}This is the third line.first", tv.Text);
		Assert.Equal (new Point (28, 1), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		Assert.False (tv.Selecting);
		Assert.False (tv.Selecting); Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.Backspace | ConsoleDriverKey.CtrlMask)));
		...
		Assert.Equal (new Point (0, 0), tv.CursorPosition);
		Assert.False (tv.Selecting);
		Assert.False (tv.ProcessKeyPressed (new (ConsoleDriverKey.Enter)));
		Assert.Equal ($"This is the second line.{Environment.NewLine}This is the third ", tv.Text);
		Assert.Equal (new Point (0, 0), tv.CursorPosition);
		Assert.Equal (0, tv.SelectedLength);
		Assert.Equal ("", tv.SelectedText);
		...
		Assert.False (tv.Selecting);
		Assert.True (tv.AllowsReturn);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.CtrlMask | ConsoleDriverKey.End | ConsoleDriverKey.ShiftMask)));
		Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third ", tv.Text);
		Assert.Equal (new Point (18, 2), tv.CursorPosition);
		...
		Assert.True (tv.Selecting);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.CtrlMask | ConsoleDriverKey.Home | ConsoleDriverKey.ShiftMask)));
		Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third ", tv.Text);
		Assert.Equal (new Point (0, 0), tv.CursorPosition);
		Assert.Equal (Environment.NewLine.Length, tv.SelectedLength);
		Assert.Equal ($"{Environment.NewLine}", tv.SelectedText);
		Assert.True (tv.Selecting);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.T | ConsoleDriverKey.CtrlMask)));
		Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third ", tv.Text);
		Assert.Equal (new Point (18, 2), tv.CursorPosition);
		...
		Assert.True (tv.Selecting);
		Assert.True (tv.Used);
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.InsertChar)));
		Assert.False (tv.Used);
		Assert.True (tv.AllowsTab);
		Assert.Equal (new Point (18, 2), tv.CursorPosition);
		tv.AllowsTab = false;
		Assert.False (tv.ProcessKeyPressed (new (ConsoleDriverKey.Tab)));
		Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third ", tv.Text);
		...
		Assert.True (tv.Selecting);
		tv.Selecting = false;
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.Tab)));
		Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third \t", tv.Text);
		Assert.True (tv.AllowsTab);
		tv.AllowsTab = false;
		Assert.False (tv.ProcessKeyPressed (new (ConsoleDriverKey.Tab | ConsoleDriverKey.ShiftMask)));
		Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third \t", tv.Text);
		Assert.False (tv.AllowsTab);
		tv.AllowsTab = true;
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.Tab | ConsoleDriverKey.ShiftMask)));
		Assert.Equal ($"{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third ", tv.Text);
		Assert.True (tv.AllowsTab);
		Assert.False (tv.ProcessKeyPressed (new (ConsoleDriverKey.Tab | ConsoleDriverKey.CtrlMask)));
		Assert.False (tv.ProcessKeyPressed (new (Application.AlternateForwardKey)));
		Assert.False (tv.ProcessKeyPressed (new (ConsoleDriverKey.Tab | ConsoleDriverKey.CtrlMask | ConsoleDriverKey.ShiftMask)));
		Assert.False (tv.ProcessKeyPressed (new (Application.AlternateBackwardKey)));
	}
	...
	public void HistoryText_Undo_Redo_Multi_Line_With_Empty_Text ()
	{
		...
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.O | ConsoleDriverKey.ShiftMask)));
		Assert.Equal ("O", tv.Text);
		Assert.Equal (1, tv.Lines);
		Assert.Equal (new Point (1, 0), tv.CursorPosition);
		Assert.True (tv.IsDirty);

		Assert.True (tv.ProcessKeyPressed (new ((ConsoleDriverKey)'n')));
		Assert.Equal ("On", tv.Text);
		Assert.Equal (1, tv.Lines);
		Assert.Equal (new Point (2, 0), tv.CursorPosition);
		Assert.True (tv.IsDirty);

		Assert.True (tv.ProcessKeyPressed (new ((ConsoleDriverKey)'e')));
		...
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.T | ConsoleDriverKey.ShiftMask)));
		Assert.Equal ($"One{Environment.NewLine}T", tv.Text);
		Assert.Equal (2, tv.Lines);
		Assert.Equal (new Point (1, 1), tv.CursorPosition);
		Assert.True (tv.IsDirty);

		Assert.True (tv.ProcessKeyPressed (new ((ConsoleDriverKey)'w')));
		Assert.Equal ($"One{Environment.NewLine}Tw", tv.Text);
		Assert.Equal (2, tv.Lines);
		Assert.Equal (new Point (2, 1), tv.CursorPosition);
		Assert.True (tv.IsDirty);

		Assert.True (tv.ProcessKeyPressed (new ((ConsoleDriverKey)'o')));
	...
	public void HistoryText_Undo_Redo_Multi_Line_Selected_With_Empty_Text ()
	{
		...
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.D1)));
		...
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.D2)));
	...
	public void HistoryText_Undo_Redo_Multi_Line_Selected_DeleteCharLeft_All ()
	{
		...
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.End | ConsoleDriverKey.CtrlMask | ConsoleDriverKey.ShiftMask)));
		Assert.Equal ($"This is the first line.{Environment.NewLine}This is the second line.{Environment.NewLine}This is the third line.", tv.Text);
	...
	public void HistoryText_Undo_Redo_KillWordBackward ()
	{
		...
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.Backspace | ConsoleDriverKey.CtrlMask)));
		Assert.Equal ($"First line.{Environment.NewLine}Second line", tv.Text);
		Assert.Equal ("", tv.SelectedText);
		Assert.Equal (2, tv.Lines);
	...
	public void HistoryText_Undo_Redo_Changing_On_Middle_Clear_History_Forwards ()
	{
		...
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.D3)));
		...
		Assert.True (tv.ProcessKeyPressed (new (ConsoleDriverKey.D4)));
	...
	public void HistoryText_Undo_Redo_First_Line_Selected_Return_And_InsertText ()
	{
		...
		Assert.True (tv.ProcessKeyPressed (new ((ConsoleDriverKey)'a')));
	...
	public void ScrollDownTillCaretOffscreen_ThenType ()
	{
		...
		tv.ProcessKeyPressed (new (ConsoleDriverKey.P));
	}
...
└─────────────┘", _output);
	...
	public void ContentsChanged_Event_Fires_On_Typing ()
	{
		...
		expectedCol = 1;
		tv.ProcessKeyPressed (new (ConsoleDriverKey.Y | ConsoleDriverKey.ShiftMask));
	...
	public void ContentsChanged_Event_Fires_On_Undo_Redo ()
	{
		...
		expectedEventCount++;
		Assert.True (_textView.ProcessKeyPressed (new (ConsoleDriverKey.Enter)));
		Assert.Equal (expectedEventCount, eventcount);

		// Undo
		expectedEventCount++;
		Assert.True (_textView.ProcessKeyPressed (new (ConsoleDriverKey.Z | ConsoleDriverKey.CtrlMask)));
		Assert.Equal (expectedEventCount, eventcount);

		// Redo
		expectedEventCount++;
		Assert.True (_textView.ProcessKeyPressed (new (ConsoleDriverKey.R | ConsoleDriverKey.CtrlMask)));
		Assert.Equal (expectedEventCount, eventcount);
